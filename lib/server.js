...
*/
const storage = (
	{ getItem: () => {}, setItem: () => {}, init: () => Promise.resolve() }
);

const DBOpenRequest = indexedDB.open("toDoList", 1);
DBOpenRequest.onerror = (event) => { console.error("E", event); };
DBOpenRequest.onsuccess = (event) => { console.log("S", event); };

// const xdgAppPaths = XDGAppPaths({ name: 'slp-db' });
// const dataDir = xdgAppPaths.data();
const dbPath = './db.v1'; // path.join(dataDir, 'db.v1');
